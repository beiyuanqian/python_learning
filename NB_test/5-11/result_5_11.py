...
# 上报数据源
logPath = r"F:\刘小涵\测试项目\门锁\5-11数据\5-11日志上报数据.xlsx"
dic1 = result.redExcel(logPath, 0)
print('上传数据源\n', dic1)
print('上传数据源总数为：', len(dic1))

# 下发数据源
logPath = r"F:\刘小涵\测试项目\门锁\\5-11数据\\5-11日志下发数据.xlsx"
...
# result.sum3_1(dic1)
# sumAll_3_1 = result.transform_array(result.sum_3_1, 3)
# # print(sumAll_3_1)
# df3_1 = pd.DataFrame(sumAll_3_1,
#                      columns=['imei', "上报服务标识", "上报时间"])  # 使用pd.DataFrame对dataAll创建一个矩阵数据，columns为表头，dataAll为表的数据
# # 查看门锁状态上报服务标识的离散情况
# result.sum3_2(dic1)
# sumAll_3_2 = result.transform_array(result.sum_3_2, 3)
# # print(sumAll_3_2)
# df3_2 = pd.DataFrame(sumAll_3_2,
#                      columns=['imei', "上报服务标识", "上报时间"])  # 使用pd.DataFrame对dataAll创建一个矩阵数据，columns为表头，dataAll为表的数据
# # 查看开门记录上报服务标识的离散情况
# result.sum3_3(dic1)
# sumAll_3_3 = result.transform_array(result.sum_3_3, 3)
# # print(sumAll_3_3)
# df3_3 = pd.DataFrame(sumAll_3_3,
#                      columns=['imei', "上报服务标识", "上报时间"])  # 使用pd.DataFrame对dataAll创建一个矩阵数据，columns为表头，dataAll为表的数据
# # 查看门锁告警上报服务标识的离散情况
# result.sum3_4(dic1)
# sumAll_3_4 = result.transform_array(result.sum_3_4, 3)
# # print(sumAll_3_4)
# df3_4 = pd.DataFrame(sumAll_3_4,
#                      columns=['imei', "上报服务标识", "上报时间"])  # 使用pd.DataFrame对dataAll创建一个矩阵数据，columns为表头，dataAll为表的数据
...
# 保存到本地excel
with pd.ExcelWriter(r'F:\刘小涵\测试项目\门锁\5-11数据\5-11数据对比结果汇总.xlsx') as writer:
    ...
    # df3_1.to_excel(writer, sheet_name='密码更新和NFC更新时间')
    # df3_2.to_excel(writer, sheet_name='门锁状态上报时间')
    # df3_3.to_excel(writer, sheet_name='开门记录上报时间')
    # df3_4.to_excel(writer, sheet_name='门锁告警上报时间')
